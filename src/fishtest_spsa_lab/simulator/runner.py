...
class SpsaRunner:
    ...
    def run(self) -> dict:
        ...
        num_active = np.count_nonzero(self.config.w_true)
        logger.info(
            "Target parameters: %s (active: %s)",
            self.config.theta_peak[0],  # Just show first one as example
            num_active,
        )
...
class AsyncSpsaRunner(SpsaRunner):
    ...
    def _schedule_job(self, worker_id: int) -> None:
        ...
        # Attach batch size via a dynamic attribute
        event.batch_size_pairs = batch_size_pairs
    ...
    def run(self) -> dict:
        ...
        logger.info(
            "Target parameters: %s (active: %s)",
            self.config.theta_peak[0],
            num_active,
        )

        logger.info(
            "Initial Theta (first 5): %s",
            self.optimizer.get_params()[:5],
        )
        logger.info(
            "Initial Elo: %.2f",
            objective_function(self.optimizer.get_params(), self.config),
        ...
        # Collect results using the shared metrics helper
